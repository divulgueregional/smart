...
const createSession = async (sessionId, isLegacy = false, res = null) => {
    ...
    wa.ev.on('messages.upsert', async (m) => {
        ...
        //sendMessage(44,message.key.remoteJid, { text: 'string' })
        if (!message.key.fromMe && m.type === 'notify') {
            //sendMessage(message.key.remoteJid, { text: 'string' })
            await delay(1000)
        ...
        if (update.qr) {
            if (res && !res.headersSent) {
                try {
                    ...
                    response(res, 200, true, 'QR code recebido, por favor escanear o QR code.', { qr })
                } catch {
                    response(res, 500, false, 'Não foi possível criar o código QR.')
                }
...
const cleanup = () => {
    console.log('Executando a limpeza antes de sair.')
